module uii2c#
(
    parameter WMEN_LEN = 4,
    parameter RMEN_LEN = 1,
...
(
    ...
    output reg iic_scl,
    inout iic_sda,
    input [(WMEN_LEN*8 - 1'b1):0] wr_data,          // 待写数据
    input [7:0] wr_cnt,                             // 待写数据长度（含设备地址，单位：字节） 
    output reg [(RMEN_LEN*8 - 1'b1):0] rd_data,     // 待读数据   
    input [7:0] rd_cnt,                             // 待读数据长度（单位：字节）
    input iic_en,                                   // IIC 数据传输使能（高电平有效）
    input iic_mode,                                 // IIC 传输模式（0：随机传输；1：按页传输）
    output reg iic_busy                             // IIC 繁忙标志（高电平有效）
);
    ...
    wire sda_i;
    wire scl_offset  = (clk_div_cnt == OFFSET);
